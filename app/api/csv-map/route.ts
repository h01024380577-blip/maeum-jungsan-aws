...
2. 한 열을 두 항목에 동시 매핑하지 말 것.
3. 헤더 이름이 모호하면 sampleRows 의 값 패턴으로 판단 (예: 숫자만 → amount, YYYY-MM-DD/2024.5.3 → date).
4. confidence: high(헤더+샘플 모두 명확) / medium(헤더만 또는 샘플만) / low(추측).
5. 보냄/받음(수입/지출) 은 절대 추론하지 말 것 — 사용자가 직접 선택한다. reason 에도 지출/수입 판단을 언급하지 마.
...
출력 형식 (JSON, 다른 텍스트 금지):
{
  ...
  "reason": "<짧은 한국어 설명, 50자 이내. 컬럼 매칭 근거만 적기.>"
}`;
